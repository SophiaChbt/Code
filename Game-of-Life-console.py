...
def next_generation(tab): # Calcul des g�n�ration de la colonie
    global arret    
    copie = deepcopy(tab)
    t = draft(copie)
    for i in range(n):
...
def jeu_de_la_vie(): # Calcul et affichage des g�n�rations
    global arret
    arret = False
    while arret == False:
        if input("n for Next Generation: ")=="n":
            ...
            affichage(tab)
    if arret == True:
        print("La colonie n'�volue plus, entrez 'jeu_de_la_vie()' pour recommencer le jeu :D")
